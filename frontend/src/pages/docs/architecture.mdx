...
graph TD
    ...
    Proxy -- "/_next, /*" --> NextJS
    Proxy -- "/api/*" --> Axum
    NextJS -. "Next.js Rewrites<br/>(next.config.ts)" .-> Axum
...
*   **Proxying (CORS Bypass):** As defined in `frontend/next.config.ts`, requests to `/api/*` on the Next.js dev server are rewritten to `http://localhost:8080/api/*`. In production (`Caddyfile`), Caddy routes `/api/*` directly to the Rust container.
